import React, { Component } from 'react';
import { NavLink } from 'react-router-dom';
import './file.css';

const File = (props) => {

    const handleFileClick = () => {
        
    }

    return ( 
        <div className="col">
            <NavLink to={`/fileInfo/${props.info.Hospital}/${props.info.name}`}><svg xmlns="http://www.w3.org/2000/svg" width="100%" height="200" fill="currentColor" class="bi bi-file-earmark-diff-fill file-icon" viewBox="0 0 16 16" onClick={handleFileClick}>
            <path d="M9.293 0H4a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V4.707A1 1 0 0 0 13.707 4L10 .293A1 1 0 0 0 9.293 0zM9.5 3.5v-2l3 3h-2a1 1 0 0 1-1-1zM8 6a.5.5 0 0 1 .5.5V8H10a.5.5 0 0 1 0 1H8.5v1.5a.5.5 0 0 1-1 0V9H6a.5.5 0 0 1 0-1h1.5V6.5A.5.5 0 0 1 8 6zm-2.5 6.5A.5.5 0 0 1 6 12h4a.5.5 0 0 1 0 1H6a.5.5 0 0 1-.5-.5z"/>
            </svg></NavLink>
            <div className="file-desc">
                {props.info.name} 
                <br />
                ({props.info.Hospital})
                <hr></hr>
            </div>
        </div>
     );
}
 
export default File;